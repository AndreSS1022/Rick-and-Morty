...
const displayCharacter = characters => {
    ...
    characters.forEach(character => {
        ...
        console.log(character.id.image)
        const characterDiv = document.createElement("div")
        characterDiv.className = "single-result row align-items-center my-3 p-3"
        characterDiv.innerHTML = `
        
                    <div class="col-lg-2 text-center">
                        <img class src=${character.id.image} style="height:50px ;" alt=""/>
                    </div>
                    <div class="col-lg-4 text-center">
                        <h3 class="lyrics-name">${character.name}</h3>
                        <p class="author lead">Album by <span>${character.id.image}</span></p>
                    </div>
